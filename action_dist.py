...
class TorchMultinomialAutoregressiveDistribution(TorchDistributionWrapper):
    ...
    def deterministic_sample(self):
        """Deterministically sample action 1 (buy/sell/hold) and then action 2 (amount) conditioned action 1."""
    ...
    def sample(self):
        """Sample action 1 (buy/sell/hold) and then action 2 (amount) conditioned action 1."""
        a1_dist = self._a1_distribution()
#        try:
        a1 = a1_dist.sample()
#        except:
...
#            a1 = a1_dist.sample()
        ...
        if torch.isnan(a1_dist.inputs).sum().item() > 0:
            print('reg')
            ray.util.pdb.set_trace()        
        
        if torch.isnan(a2_dist.inputs).sum().item() > 0:
            print('reg')
            ray.util.pdb.set_trace()
    ...
    def kl(self, other):
        """Calculates the KL divergence between the old and updated action distributions."""
        a1_dist = self._a1_distribution()
    ...
    def _a1_distribution(self):
        ...
        # may need to set .to(device) if transitioning to GPU
        # see source
